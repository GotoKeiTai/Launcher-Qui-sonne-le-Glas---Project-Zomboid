...
- **Tension identifiée et résolue** : rendre l'option de lancement pilotée par le manifeste la rendrait dépendante de l'hébergement (qui n'existe pas encore) — ce qui annulerait la correction précédente (review finale du sous-projet #2) qui gardait volontairement ce check *indépendant* du manifeste pour rester observable. **Résolution : approche hybride** — une valeur par défaut codée en dur (repli) est utilisée tant que le manifeste ne fournit rien ; le manifeste prend le dessus automatiquement dès qu'il est disponible. (Le repli vit dans `JavaModService`, pas `SteamEnvironment` — voir "Architecture" ci-dessous ; ajusté après la review finale de ce même sous-projet, qui a repéré que le séparer de `SteamEnvironment` produisait un message vide au joueur tant qu'aucun manifeste n'est disponible.)
