...
- **Prochain chantier convenu** : remplacer les Fakes restants par de vrais services Windows, dans cet ordre (chacun avec son propre cycle spec → plan → implémentation) :
  ...
  2. **Gestion du mod Java / agent** — implémentation réelle d'`IJavaModService` (présence/version/SHA-256, lecture `localconfig.vdf`, vrai téléchargement+installation de l'agent ZombieBuddy + `GlasJavaMod.jar`). Dépend de #1 (dossier d'installation du jeu, maintenant détecté). **À reprendre ensuite** — pas encore brainstormé, aucune spec écrite.
  3. **Packaging Velopack & CI** — nature différente (infra de build/release, pas un service runtime). Probablement en dernier.
  4. *(Non planifié formellement mais nécessaire à terme)* **Vrai `IServerInfoService`** — fetch de `server.json`/`mods.json` (buildid requis, IDs Workshop requis, changelog, news) depuis l'hébergement statique (§8.3 du cahier des charges). Sans ça, `CanPlay` ne passera jamais sur une vraie machine même avec #1 et #2 terminés.
